"""
network_capture/capture.py
==========================
Real-time network packet capture using Scapy.
Extracts NSL-KDD compatible features from live TCP/UDP/ICMP flows
and submits them to the Django IDS API automatically.
...
# ── TCP Flag mapping to NSL-KDD flag codes ─────────────────────────────────
def get_tcp_flag(flags):
    """Map Scapy TCP flags to NSL-KDD flag string."""
    if not flags:
        return 'OTH'
    f = int(flags)
    SYN = bool(f & 0x02)
    ...
    if SYN and not ACK:  return 'S0'   # SYN sent, no response
    if SYN and ACK:      return 'SF'   # Normal established + closed
...
# ── Port → Service mapping (simplified NSL-KDD services) ───────────────────
PORT_SERVICE = {
...
# ── Flow record (accumulates packets in a connection) ──────────────────────
class FlowRecord:
    TIMEOUT = 30  # seconds of inactivity before flow is finalised

    def __init__(self, key: FlowKey, timestamp: float):
        self.key       = key
        self.start_ts  = timestamp
        self.last_ts   = timestamp
        self.packets   = []          # list of (timestamp, size, flags, is_reverse)
    ...
    def finalise(self, recent_flows: list) -> dict:
        """Convert flow to NSL-KDD feature dict."""
        proto = self.key.proto
        service = port_to_service(self.key.dst_port, proto)
        flag = self.last_flag

        # Time-window stats (last 2 seconds among recent_flows)
        now = time.time()
        ...
        # Dst-host window (last 100 connections to same dst host)
        dst_host_flows = [
        ...
        return {
            ...
            'urgent':             self.urgent,
            # Content features (hard to derive without DPI)
            'hot':                0,
            'num_failed_logins':  0,
...
class FlowManager:
    def __init__(self, api_url: str, submit_interval: int = 5):
        ...
        self.active_flows    = {}        # FlowKey → FlowRecord
        self.completed_flows = []        # recently finished flows
    ...
    def process_packet(self, pkt):
        ...
        with self.lock:
            ...
            # Check if this is a reverse packet (response)
                ...
                if resp.status_code == 201:
                    ...
                    if pred != 'Normal':
                        ...
                        log.warning(
                            ...
                            f"Confidence: {conf}% | "
                            f"Block #{result.get('blockchain', {}).get('block_index', '?')}"
                        )
                    else:
                        log.info(
                            f"✅ Normal | "
...
def main():
    ...
    parser.add_argument('--iface',    default=None,
                        help='Network interface (e.g. eth0, en0, Wi-Fi). Default: all interfaces')
    ...
    # Start periodic submitter
    submitter = SubmitterThread(manager, args.interval)
    submitter.start()

    log.info("=" * 60)
    log.info("  IDSGuard — Real-Time Network Capture")
    log.info("=" * 60)
    ...
    try:
        ...
        log.error("❌ Permission denied. Run with: sudo python capture.py")
    except Exception as e:
